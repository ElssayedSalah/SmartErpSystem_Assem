...
namespace SmartErpApp.Controllers.Inventory
{
    ...
    public class SalesReturnController : BaseAdminController
    {
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "SalesReturn", CreateNewBtnAction = "Create", BackToListControler = "SalesReturn", BackToListAction = "Index", EditBtnControler = "SalesReturn", EditBtnAction = "Edit", DeleteBtnControler = "SalesReturn", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }
        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
             ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "SalesReturn", CreateNewBtnAction = "Create", BackToListControler = "SalesReturn", BackToListAction = "Index", EditBtnControler = "SalesReturn", EditBtnAction = "Edit", DeleteBtnControler = "SalesReturn", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "SalesReturn", PrintBtnAction = "Print", SendEInvoiceBtnControler = "SalesInvoice", SendEInvoiceBtnAction = "SendEInvoice", SendEInvoiceBtnVisibilty = true, DocumentType = "C" };
        }
            ...
            if (model != null && model.Transaction_InvDetails != null)
            {
                ...
                if (model.Transaction_InvDetails.Count > 1)
                {
                    foreach (var item in model.Transaction_InvDetails.Skip(1))
                    {
                        ...
                        item.DocTypeId = (int)DocumentTypes.SalesReturn;
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageSalesReturn.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var TrMaster = _InventoryMasterService.GetInventoryTransactions(x=>x.FinancialPeriodId== CurrentUser.FinancialPeriodId && x.DocTypeId== (int)DocumentTypes.SalesReturn && x.CompanyId== CurrentUser.CompanyId);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageSalesReturn.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView(); var LastCode = _InventoryMasterService.GetLastCode(o => o.Code ,x=> x.FinancialPeriodId==CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.SalesReturn && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] Transaction_InvMasterModel TransactionModel)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    //التأكد من الحسابات الافتراضية ومواصفات حركة مرتجع مبيعات إذا كان النظام يسمح بانشاء قيود الحركات
                    ...
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        if (!_InventoryMasterService.IsExistRecord(b => b.Code == TransactionModel.Code && b.FinancialPeriodId== currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.SalesReturn && b.CompanyId == CurrentUser.CompanyId))
                        {
                            ...
                            master.SetBasicData(SharedEnums.CRUD_OperationType.Create, (int)DocumentTypes.SalesReturn, currentUser);
                            ...
                            if (transactionSettings.EnableEntryCreation)
                            {
                                ...
                                int EntryId = 0;
                                //add invoice entry
                                AddEntry(transactionSettings, master.Code, master.CurrencyId, master.CurrencyFactor, master.InvoiceValue, master.InvoiceNet, master.TaxValue, out EntryNumber, out EntryId);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageSalesReturn.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.SalesReturn, CurrentUser);
                        ...
                        if (transactionSettings.EnableEntryCreation)
                        {
                            ...
                            {
                                UpdateEntry(transactionSettings, master.Code, master.CurrencyId, master.CurrencyFactor, master.InvoiceValue, master.InvoiceNet, master.TaxValue, master.EntryId);
                            }
                            else
                            {
                                int EntryNumber = 0;
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageSalesReturn.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                SalesReturnRptPrint report = new SalesReturnRptPrint(_LocalizationService);
                var DataSource = _ReportService.SalesReturnPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "SalesReturnRpt.pdf");
            }
            else
            {
                return View("Error");
        ...
        public string ValidateEntrySettings(TransactionEntrySetting transactionSettings, Customer customer)
        {
            ...
            if (SystemSetting != null && SystemSetting.EnableTransactionsEntryCreation)
            {
                ...
                var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.SalesReturn).FirstOrDefault();
                ...
                else
                {
                    ...
                    var FSAccount = defaultAccounts.Where(a => a.AccountNameId == 25).FirstOrDefault();
                    if (FSAccount == null || FSAccount.AccountId <= 0)
                    {
                        msg.AppendLine("يجب تحديد حساب افتراضي لحساب مردودات المبيعات <br>");
                    }
                    else
                    {
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal FirstSideAmount, decimal SecondSideAmount, decimal VAT_TaxAmount, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.SalesReturn;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.SalesReturn, CurrentUser);
                #endregion
                ...
                //الطرف الثاني لحساب مردودات المبيعات
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal FirstSideAmount, decimal SecondSideAmount, decimal VAT_TaxAmount, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.SalesReturn, CurrentUser);
                    #endregion

                    #region EntryDetails
                    //الطرف الاول لحساب مردودات المبيعات
                    DailyEntryDetails DailyEntry1 = new DailyEntryDetails();

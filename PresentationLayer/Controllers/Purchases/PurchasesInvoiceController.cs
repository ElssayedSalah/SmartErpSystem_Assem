...
namespace SmartErpApp.Controllers.Inventory
{
    ...
    public class PurchasesInvoiceController : BaseAdminController
    {
        ...
        public PurchasesInvoiceController(
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "PurchasesInvoice", CreateNewBtnAction = "Create", BackToListControler = "PurchasesInvoice", BackToListAction = "Index", EditBtnControler = "PurchasesInvoice", EditBtnAction = "Edit", DeleteBtnControler = "PurchasesInvoice", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }
        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "PurchasesInvoice", CreateNewBtnAction = "Create", BackToListControler = "PurchasesInvoice", BackToListAction = "Index", EditBtnControler = "PurchasesInvoice", EditBtnAction = "Edit", DeleteBtnControler = "PurchasesInvoice", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "PurchasesInvoice", PrintBtnAction = "Print" };
        }
            ...
            if (model != null && model.Transaction_InvDetails != null)
            {
                ...
                if (model.Transaction_InvDetails.Count > 1)
                {
                    foreach (var item in model.Transaction_InvDetails.Skip(1))
                    {
                        ...
                        item.DocTypeId = (int)DocumentTypes.PurchaseInvoice;
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManagePurchasesInvoice.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var TrMaster = _InventoryMasterService.GetInventoryTransactions(x=>x.FinancialPeriodId== CurrentUser.FinancialPeriodId && x.DocTypeId== (int)DocumentTypes.PurchaseInvoice && x.CompanyId== CurrentUser.CompanyId);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManagePurchasesInvoice.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView();
            
            var LastCode = _InventoryMasterService.GetLastCode(o => o.Code ,x=> x.FinancialPeriodId==CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.PurchaseInvoice && x.CompanyId == CurrentUser.CompanyId);
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        if (!_InventoryMasterService.IsExistRecord(b => b.Code == TransactionModel.Code && b.FinancialPeriodId== currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.PurchaseInvoice && b.CompanyId == CurrentUser.CompanyId))
                        {
                            ...
                            master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.PurchaseInvoice, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManagePurchasesInvoice.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.PurchaseInvoice, CurrentUser);
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManagePurchasesInvoice.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                PurchasesInvoiceRptPrint report = new PurchasesInvoiceRptPrint(_LocalizationService);
                var DataSource = _ReportService.PurchasesInvoicePrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "PurchasesInvoice.pdf");
            }
            else
            {
                return View("Error");
        ...
        public string ValidateEntrySettings(TransactionEntrySetting transactionSettings,Suppler supper)
        {
            ...
            if (SystemSetting!=null && SystemSetting.EnableTransactionsEntryCreation)
            {
                ...
                var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.PurchaseInvoice).FirstOrDefault();
                ...
                {
                    //الطرف الاول حساب مخزون البضاعة
                    var FSAccount = defaultAccounts.Where(a => a.AccountNameId == 2).FirstOrDefault();
                    if (FSAccount==null || FSAccount.AccountId<=0)
                    {
                        msg.AppendLine("يجب تحديد حساب افتراضي لحساب مخزون البضاعة <br>") ;
                    }
                    else
                    {
                ...
                {
                    msg.AppendLine("يجب تحديد حساب للمورد <br>");
                }
                else
                {
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings,int DocNumber, int CuruncyId,decimal CuruncyFactor,decimal FirstSideAmount,decimal SecondSideAmount,decimal VAT_TaxAmount,out int EntryNumber,out int EntryId)
        {
            ...
            if (transactionSettings!=null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.PurchaseInvoice;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.PurchaseInvoice, CurrentUser);
                #endregion

                #region EntryDetails
                //الطرف الاول لحساب مخزون البضاعة
               ...
               List<DailyEntryDetails> EntryDetails=new List<DailyEntryDetails>();
                if (oldEntryMaster!=null)
                {
                   var oldEntryDetails = _DailyEntryService.GetEntryDetails(x=>x.MasterId==EntryId);
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.PurchaseInvoice, CurrentUser);
                    #endregion

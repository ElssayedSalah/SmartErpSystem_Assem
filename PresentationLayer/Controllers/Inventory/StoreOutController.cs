...
namespace SmartErpApp.Controllers.Inventory
{
    ...
    public class StoreOutController : BaseAdminController
    {
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "StoreOut", CreateNewBtnAction = "Create", BackToListControler = "StoreOut", BackToListAction = "Index", EditBtnControler = "StoreOut", EditBtnAction = "Edit", DeleteBtnControler = "StoreOut", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }
        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "StoreOut", CreateNewBtnAction = "Create", BackToListControler = "StoreOut", BackToListAction = "Index", EditBtnControler = "StoreOut", EditBtnAction = "Edit", DeleteBtnControler = "StoreOut", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "StoreOut", PrintBtnAction = "Print" };
        }
            ...
            if (model != null && model.Transaction_InvDetails != null)
            {
                ...
                if (model.Transaction_InvDetails.Count > 1)
                {
                    foreach (var item in model.Transaction_InvDetails.Skip(1))
                    {
                        ...
                        item.DocTypeId = (int)DocumentTypes.StoreOut;
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageStoreOut.SystemName, CurrentUser, PermissionActions.List))
        ...
        public IActionResult list()
        {
            ...
            var TrMaster = _InventoryMasterService.GetInventoryTransactions(x=>x.FinancialPeriodId== CurrentUser.FinancialPeriodId&& x.DocTypeId == (int)DocumentTypes.StoreOut && x.CompanyId == CurrentUser.CompanyId);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageStoreOut.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView();
            
            var LastCode = _InventoryMasterService.GetLastCode(o => o.Code ,x=> x.FinancialPeriodId==CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.StoreOut && x.CompanyId == CurrentUser.CompanyId);
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        if (!_InventoryMasterService.IsExistRecord(b => b.Code == TransactionModel.Code && b.FinancialPeriodId== currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.StoreOut && b.CompanyId == CurrentUser.CompanyId))
                        {
                            ...
                            master.SetBasicData(SharedEnums.CRUD_OperationType.Create, (int)DocumentTypes.StoreOut, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageStoreOut.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.StoreOut, CurrentUser);
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageStoreOut.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                StoreOutRptPrint report = new StoreOutRptPrint(_LocalizationService);
                var DataSource = _ReportService.StoreOutPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "StoreOut.pdf");
            }
            else
            {
                return View("Error");

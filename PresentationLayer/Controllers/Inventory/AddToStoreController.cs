...
namespace SmartErpApp.Controllers.Inventory
{
    ...
    public class AddToStoreController : BaseAdminController
    {
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "AddToStore", CreateNewBtnAction = "Create", BackToListControler = "AddToStore", BackToListAction = "Index", EditBtnControler = "AddToStore", EditBtnAction = "Edit", DeleteBtnControler = "AddToStore", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }
        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "AddToStore", CreateNewBtnAction = "Create", BackToListControler = "AddToStore", BackToListAction = "Index", EditBtnControler = "AddToStore", EditBtnAction = "Edit", DeleteBtnControler = "AddToStore", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "AddToStore", PrintBtnAction = "Print" };
        }
            ...
            if (model != null && model.Transaction_InvDetails != null)
            {
                ...
                if (model.Transaction_InvDetails.Count > 1)
                {
                    foreach (var item in model.Transaction_InvDetails.Skip(1))
                    {
                        ...
                        item.DocTypeId = (int)DocumentTypes.AddToStore;
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageAddToStore.SystemName, CurrentUser, PermissionActions.List))
        ...
        public IActionResult list()
        {
            ...
            var TrMaster = _InventoryMasterService.GetInventoryTransactions(x=>x.FinancialPeriodId== CurrentUser.FinancialPeriodId&& x.DocTypeId == (int)DocumentTypes.AddToStore && x.CompanyId == CurrentUser.CompanyId);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageAddToStore.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView(); 
            
            var LastCode = _InventoryMasterService.GetLastCode(o => o.Code ,x=> x.FinancialPeriodId==CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.AddToStore && x.CompanyId == CurrentUser.CompanyId);
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        if (!_InventoryMasterService.IsExistRecord(b => b.Code == TransactionModel.Code && b.FinancialPeriodId== currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.AddToStore && b.CompanyId == CurrentUser.CompanyId))
                        {
                            ...
                            master.SetBasicData(SharedEnums.CRUD_OperationType.Create, (int)DocumentTypes.AddToStore, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageAddToStore.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (TransactionModel.Transaction_InvDetails != null && TransactionModel.Transaction_InvDetails.Count > 0)
                    {
                        ...
                        master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.AddToStore, CurrentUser);
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageAddToStore.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            ...
            if (Id > 0)
            {               
                AddToStoreRptPrint report = new AddToStoreRptPrint(_LocalizationService);
                var DataSource = _ReportService.AddToStorePrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "AddToStoreRpt.pdf");
            }
            else
            {
                return View("Error");

...
{
    public class CheckOutExchangeTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            Banks.Insert(0, new Bank() { Id = 0, NameAr = "اختيار بنك", NameEn = "select" });
            ViewBag.Banks = Banks.Select(x => new { x.Id, Name = _currentLanguage == "ar" ? x.NameAr : x.NameEn });
            ...
            var CheckTransactions =_CheckTransactionService.GetWithCondetion(x=>x.DocTypeId== (int)DocumentTypes.WriteCheckOutTransaction);
            CheckTransactions.Insert(0, new CheckTransaction() { Id = 0, CheckNumber = "إختار شيك" });
            ViewBag.CheckNumbers = CheckTransactions.Select(x => new {Id= x.CheckNumber, Name = x.CheckNumber });

            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction).FirstOrDefault();
        ...
        public void ValidateModel(CheckTransactionModel model)
        {
            if (model != null)
            {
                ...
                if (model.BankId <= 0)
                {
                    ModelState.AddModelError("BankId", "يجب اختيار بنك");
                }
                ...
                //منع صرف الشيك اكثر من مره
                var CheckOutExchangeTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == model.CheckNumber && x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction).FirstOrDefault();
                if (CheckOutExchangeTransaction != null)
                {
                    ModelState.AddModelError("CheckNumber", "رقم الشيك مسجل من قبل");
                }
                //منع صرف الشيك اكثر من مره
                var CheckReturnTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == model.CheckNumber && x.DocTypeId == (int)DocumentTypes.CheckReturnTransaction).FirstOrDefault();
                if (CheckReturnTransaction != null)
                {
                    ModelState.AddModelError("CheckNumber", "هذا الشيك تم إرتجاعه");
                }
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CheckOutExchangeTransaction", CreateNewBtnAction = "Create", BackToListControler = "CheckOutExchangeTransaction", BackToListAction = "Index", EditBtnControler = "CheckOutExchangeTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CheckOutExchangeTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CheckOutExchangeTransaction", CreateNewBtnAction = "Create", BackToListControler = "CheckOutExchangeTransaction", BackToListAction = "Index", EditBtnControler = "CheckOutExchangeTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CheckOutExchangeTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = false, DeleteBtnVisibilty = false, PrintBtnVisibilty = true, PrintBtnControler = "CheckOutExchangeTransaction", PrintBtnAction = "Print" };
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckOutExchangeTransaction.SystemName, CurrentUser, PermissionActions.List))
                return AccessDeniedView();

            return View(new CheckTransactionModel());
        }
        public IActionResult list()
        {
            var transactions = _CheckTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction);
            var branches = _BranchService.GetAll();
            var transactionsModel = _Mapper.Map<List<CheckTransactionModel>>(transactions);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckOutExchangeTransaction.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView();

            var LastCode = _CheckTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction && x.CompanyId == CurrentUser.CompanyId);
            var DefaultCurrency = _CurrencyService.GetWithCondetion(x => x.DefaultCurrency).FirstOrDefault();



            var Model = new CheckTransactionModel() { Code = LastCode, DocDate = DateTime.Now,DueDate = DateTime.Now, CurrencyId = DefaultCurrency != null ? DefaultCurrency.Id : 0 };
        ...
        public IActionResult Create([Bind] CheckTransactionModel model)
        {
            CheckTransaction master = null;
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckOutExchangeTransaction.SystemName, CurrentUser, PermissionActions.Create))
                    return AccessDeniedView();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CheckTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CheckTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CheckOutExchangeTransaction, currentUser);
                        ...
                        _CheckTransactionService.Add(master);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckOutExchangeTransaction.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
            CheckTransaction master = _CheckTransactionService.GetById(Id);

            CheckTransactionModel masterModel = _Mapper.Map<CheckTransactionModel>(master);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Edit(CheckTransactionModel model)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckOutExchangeTransaction.SystemName, CurrentUser, PermissionActions.Edit))
                ...
                if (ModelState.IsValid)
                {
                    ...
                    CheckTransaction master = _Mapper.Map<CheckTransaction>(model);
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.CheckOutExchangeTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
                    ...
                    _CheckTransactionService.Update(master);
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckOutExchangeTransaction.SystemName, CurrentUser, PermissionActions.Delete))
                    return Json(new { type = "faild", msg = _LocalizationService.GetLocalizedHtmlString("Permission.AccessDenied").Value });

                CheckTransaction master = _CheckTransactionService.GetById(Id);
                _CheckTransactionService.Delete(master);
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                CheckOutExchangeRptPrint report = new CheckOutExchangeRptPrint(_LocalizationService);
                var DataSource = _ReportService.CheckOutExchangeRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "CheckOutExchangeRptPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public IActionResult GetCheckDetails(string CheckNumber)
        {
            
            var CheckTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == CheckNumber && x.DocTypeId== (int)DocumentTypes.WriteCheckOutTransaction).FirstOrDefault();            
            return Json(CheckTransaction);
        }
        ...
        public string ValidateEntrySettings(TransactionEntrySetting transactionSettings, CheckTransactionModel model)
        {
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.CheckOutExchangeTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CheckOutExchangeTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.CheckOutExchangeTransaction, CurrentUser);
                    #endregion

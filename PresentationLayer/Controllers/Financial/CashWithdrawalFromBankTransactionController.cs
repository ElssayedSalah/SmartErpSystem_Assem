...
{
    public class CashWithdrawalFromBankTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CashWithdrawalFromBankTransaction).FirstOrDefault();
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CashWithdrawalFromBankTransaction", CreateNewBtnAction = "Create", BackToListControler = "CashWithdrawalFromBankTransaction", BackToListAction = "Index", EditBtnControler = "CashWithdrawalFromBankTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CashWithdrawalFromBankTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CashWithdrawalFromBankTransaction", CreateNewBtnAction = "Create", BackToListControler = "CashWithdrawalFromBankTransaction", BackToListAction = "Index", EditBtnControler = "CashWithdrawalFromBankTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CashWithdrawalFromBankTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "CashWithdrawalFromBankTransaction", PrintBtnAction = "Print"};
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashWithdrawalFromBankTransaction.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var transactions = _CashTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.CashWithdrawalFromBankTransaction);
            var branches = _BranchService.GetAll();
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashWithdrawalFromBankTransaction.SystemName, CurrentUser, PermissionActions.Create))
               return AccessDeniedView();

            var LastCode = _CashTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.CashWithdrawalFromBankTransaction && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] CashTransactionModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CashTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.CashWithdrawalFromBankTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CashTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CashWithdrawalFromBankTransaction, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashWithdrawalFromBankTransaction.SystemName, CurrentUser, PermissionActions.Edit))
             return AccessDeniedView();
        ...
        public IActionResult Edit(CashTransactionModel model)
        {
            try
            {
                ...
                {
                    TransactionEntrySetting transactionSettings = new TransactionEntrySetting();
                    //التأكد من الحسابات الافتراضية ومواصفات حركة الحركة إذا كان النظام يسمح بانشاء قيود الحركات
                    var msg = ValidateEntrySettings(transactionSettings, model);
                    ...
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.CashWithdrawalFromBankTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCashWithdrawalFromBankTransaction.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                CashWithdrawalFromBankRptPrint report = new CashWithdrawalFromBankRptPrint(_LocalizationService);
                var DataSource = _ReportService.CashWithdrawalFromBankRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "CashWithdrawalFromBankRptPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.CashWithdrawalFromBankTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CashWithdrawalFromBankTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.CashWithdrawalFromBankTransaction, CurrentUser);
                    #endregion

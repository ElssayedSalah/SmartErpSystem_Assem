...
{
    public class CashExchangeTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CashExchangeTransaction).FirstOrDefault();
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CashExchangeTransaction", CreateNewBtnAction = "Create", BackToListControler = "CashExchangeTransaction", BackToListAction = "Index", EditBtnControler = "CashExchangeTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CashExchangeTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CashExchangeTransaction", CreateNewBtnAction = "Create", BackToListControler = "CashExchangeTransaction", BackToListAction = "Index", EditBtnControler = "CashExchangeTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CashExchangeTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "CashExchangeTransaction", PrintBtnAction = "Print"};
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashExchangeTransaction.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var transactions = _CashTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.CashExchangeTransaction);
            var branches = _BranchService.GetAll();
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashExchangeTransaction.SystemName, CurrentUser, PermissionActions.Create))
               return AccessDeniedView();

            var LastCode = _CashTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.CashExchangeTransaction && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] CashTransactionModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CashTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.CashExchangeTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CashTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CashExchangeTransaction, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashExchangeTransaction.SystemName, CurrentUser, PermissionActions.Edit))
             return AccessDeniedView();
        ...
        public IActionResult Edit(CashTransactionModel model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.CashExchangeTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCashExchangeTransaction.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                CashExchangeRptPrint report = new CashExchangeRptPrint(_LocalizationService);
                var DataSource = _ReportService.CashExchangeRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "CashExchangeRptPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.CashExchangeTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CashExchangeTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.CashExchangeTransaction, CurrentUser);
                    #endregion

...
{
    public class CheckInExchangeTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            var CheckTransactions =_CheckTransactionService.GetWithCondetion(x=>x.DocTypeId== (int)DocumentTypes.RecieveCheckInTransaction);
            ...
            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CheckInExchangeTransaction).FirstOrDefault();
        ...
        public void ValidateModel(CheckTransactionModel model)
        {
            if (model != null)
            {
                ...
                var CheckTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == model.CheckNumber && x.DocTypeId == (int)DocumentTypes.CheckInExchangeTransaction).FirstOrDefault();
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CheckInExchangeTransaction", CreateNewBtnAction = "Create", BackToListControler = "CheckInExchangeTransaction", BackToListAction = "Index", EditBtnControler = "CheckInExchangeTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CheckInExchangeTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CheckInExchangeTransaction", CreateNewBtnAction = "Create", BackToListControler = "CheckInExchangeTransaction", BackToListAction = "Index", EditBtnControler = "CheckInExchangeTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CheckInExchangeTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = false, DeleteBtnVisibilty = false, PrintBtnVisibilty = true, PrintBtnControler = "CheckInExchangeTransaction", PrintBtnAction = "Print" };
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckInExchangeTransaction.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var transactions = _CheckTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.CheckInExchangeTransaction);
            var branches = _BranchService.GetAll();
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckInExchangeTransaction.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView();

            var LastCode = _CheckTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.CheckInExchangeTransaction && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] CheckTransactionModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CheckTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.CheckInExchangeTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CheckTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CheckInExchangeTransaction, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckInExchangeTransaction.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
        ...
        public IActionResult Edit(CheckTransactionModel model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.CheckInExchangeTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckInExchangeTransaction.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                CheckInExchangeRptPrint report = new CheckInExchangeRptPrint(_LocalizationService);
                var DataSource = _ReportService.CheckInExchangeRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "CheckInExchangeRptPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public IActionResult GetCheckDetails(string CheckNumber)
        {
            ...
            var CheckTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == CheckNumber && x.DocTypeId== (int)DocumentTypes.RecieveCheckInTransaction).FirstOrDefault();            
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.CheckInExchangeTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CheckInExchangeTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.CheckInExchangeTransaction, CurrentUser);
                    #endregion

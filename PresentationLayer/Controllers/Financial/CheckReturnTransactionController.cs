...
{
    public class CheckReturnTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            if (CheckTransactions!=null && CheckTransactions.Count>0)
            {
                //كتابة شيكات واردة لم يتم اعتمادها في حركة صرف شيك صادر
                var ReservedCheckTransactions = _CheckTransactionService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction).Select(x=>x.CheckNumber).ToList();

                CheckTransactions = CheckTransactions.Where(x => !ReservedCheckTransactions.Contains(x.CheckNumber)).ToList() ;
            ...
            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CheckReturnTransaction).FirstOrDefault();
        ...
        public void ValidateModel(CheckTransactionModel model)
        {
            if (model != null)
            {
                ...
                var CheckTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == model.CheckNumber && x.DocTypeId == (int)DocumentTypes.CheckReturnTransaction).FirstOrDefault();
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CheckReturnTransaction", CreateNewBtnAction = "Create", BackToListControler = "CheckReturnTransaction", BackToListAction = "Index", EditBtnControler = "CheckReturnTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CheckReturnTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CheckReturnTransaction", CreateNewBtnAction = "Create", BackToListControler = "CheckReturnTransaction", BackToListAction = "Index", EditBtnControler = "CheckReturnTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CheckReturnTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = false, DeleteBtnVisibilty = false, PrintBtnVisibilty = true, PrintBtnControler = "CheckReturnTransaction", PrintBtnAction = "Print" };
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckReturnTransaction.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var transactions = _CheckTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.CheckReturnTransaction);
            var branches = _BranchService.GetAll();
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckReturnTransaction.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView();

            var LastCode = _CheckTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.CheckReturnTransaction && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] CheckTransactionModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CheckTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.CheckReturnTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CheckTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CheckReturnTransaction, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckReturnTransaction.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
        ...
        public IActionResult Edit(CheckTransactionModel model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.CheckReturnTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCheckReturnTransaction.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                CheckReturnRptPrint report = new CheckReturnRptPrint(_LocalizationService);
                var DataSource = _ReportService.CheckReturnRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "CheckReturnRptPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.CheckReturnTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CheckReturnTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.CheckReturnTransaction, CurrentUser);
                    #endregion

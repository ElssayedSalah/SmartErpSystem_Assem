...
{
    public class CashDepositInBankTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.CashDepositInBankTransaction).FirstOrDefault();
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CashDepositInBankTransaction", CreateNewBtnAction = "Create", BackToListControler = "CashDepositInBankTransaction", BackToListAction = "Index", EditBtnControler = "CashDepositInBankTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CashDepositInBankTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "CashDepositInBankTransaction", CreateNewBtnAction = "Create", BackToListControler = "CashDepositInBankTransaction", BackToListAction = "Index", EditBtnControler = "CashDepositInBankTransaction", EditBtnAction = "Edit", DeleteBtnControler = "CashDepositInBankTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "CashDepositInBankTransaction", PrintBtnAction = "Print"};
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashDepositInBankTransaction.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var transactions = _CashTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.CashDepositInBankTransaction);
            var branches = _BranchService.GetAll();
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashDepositInBankTransaction.SystemName, CurrentUser, PermissionActions.Create))
               return AccessDeniedView();

            var LastCode = _CashTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.CashDepositInBankTransaction && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] CashTransactionModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CashTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.CashDepositInBankTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CashTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CashDepositInBankTransaction, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageCashDepositInBankTransaction.SystemName, CurrentUser, PermissionActions.Edit))
             return AccessDeniedView();
        ...
        public IActionResult Edit(CashTransactionModel model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.CashDepositInBankTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageCashDepositInBankTransaction.SystemName, CurrentUser, PermissionActions.Delete))
        ...
        {            
            ...
            if (Id > 0)
            {
                CashDepositInBankRptPrint report = new CashDepositInBankRptPrint(_LocalizationService);
                var DataSource = _ReportService.CashDepositInBankRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "CashDepositInBankPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.CashDepositInBankTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.CashDepositInBankTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.CashDepositInBankTransaction, CurrentUser);
                    #endregion

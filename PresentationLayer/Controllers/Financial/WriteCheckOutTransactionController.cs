...
{
    public class WriteCheckOutTransactionController : BaseAdminController
    {
        ...
        public void IntializeDropdowens()
        {
            ...
            var transSettings = _TransactionsEntrySettingService.GetWithCondetion(x => x.DocTypeId == (int)DocumentTypes.WriteCheckOutTransaction).FirstOrDefault();
        ...
        public void ValidateModel(CheckTransactionModel model)
        {
            if (model != null)
            {
                ...
                if (model.CheckNumber !=null && model.CheckNumber!="")
                {
                    var CheckTransaction = _CheckTransactionService.GetWithCondetion(x=>x.CheckNumber==model.CheckNumber && x.Id!=model.Id && x.DocTypeId == (int)DocumentTypes.WriteCheckOutTransaction).FirstOrDefault();
        ...
        public void SetBasicButtonsVisibilityForCreate()
        {
            ...
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "WriteCheckOutTransaction", CreateNewBtnAction = "Create", BackToListControler = "WriteCheckOutTransaction", BackToListAction = "Index", EditBtnControler = "WriteCheckOutTransaction", EditBtnAction = "Edit", DeleteBtnControler = "WriteCheckOutTransaction", DeleteBtnAction = "Delete", RouteId = 0, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = false, PrintBtnVisibilty = false };

        }

        public void SetBasicButtonsVisibilityForEdit(int RouteId)
        {
            ViewData["BasicButtons"] = new BasicButtons() { CreateNewBtnControler = "WriteCheckOutTransaction", CreateNewBtnAction = "Create", BackToListControler = "WriteCheckOutTransaction", BackToListAction = "Index", EditBtnControler = "WriteCheckOutTransaction", EditBtnAction = "Edit", DeleteBtnControler = "WriteCheckOutTransaction", DeleteBtnAction = "Delete", RouteId = RouteId, CreateNewBtnVisibilty = true, EditBtnVisibilty = false, BackToListBtnVisibilty = true, SaveBtnVisibilty = true, DeleteBtnVisibilty = true, PrintBtnVisibilty = true, PrintBtnControler = "WriteCheckOutTransaction", PrintBtnAction = "Print" };
        }
        public IActionResult Index()
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageWriteCheckOutTransaction.SystemName, CurrentUser, PermissionActions.List))
        ...
        {
            var transactions = _CheckTransactionService.GetWithCondetion(x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.CompanyId == CurrentUser.CompanyId && x.DocTypeId == (int)DocumentTypes.WriteCheckOutTransaction);
            var branches = _BranchService.GetAll();
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageWriteCheckOutTransaction.SystemName, CurrentUser, PermissionActions.Create))
                return AccessDeniedView();

            var LastCode = _CheckTransactionService.GetLastCode(o => o.Code, x => x.FinancialPeriodId == CurrentUser.FinancialPeriodId && x.DocTypeId == (int)DocumentTypes.WriteCheckOutTransaction && x.CompanyId == CurrentUser.CompanyId);
        ...
        public IActionResult Create([Bind] CheckTransactionModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (!_CheckTransactionService.IsExistRecord(b => b.Code == model.Code && b.FinancialPeriodId == currentUser.FinancialPeriodId && b.DocTypeId == (int)DocumentTypes.WriteCheckOutTransaction && b.CompanyId == CurrentUser.CompanyId))
                    {
                        master = _Mapper.Map<CheckTransaction>(model);
                        master.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.WriteCheckOutTransaction, currentUser);
        ...
        {
            if (!_identityService.Authorize(StandardPermissionProvider.ManageWriteCheckOutTransaction.SystemName, CurrentUser, PermissionActions.Edit))
                return AccessDeniedView();
        ...
        public IActionResult Edit(CheckTransactionModel model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                   ...
                   //منع تعديل حركة كتابة شيك اذا كان تم اعتمادها في حركة صرف شيك
                    var CheckOutTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == model.CheckNumber && x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction).FirstOrDefault();
                    if (CheckOutTransaction != null)
                    {
                        msg= "غير مسموح بتعديل الحركة لأنه تم إعتمادها في حركة صرف شيك صادر";
                    }
                    ...
                    master.SetBasicData(SharedEnums.CRUD_OperationType.Update, (int)DocumentTypes.WriteCheckOutTransaction, CurrentUser); var SystemSetting = _SystemSettingService.GetAll().FirstOrDefault();
        ...
        public IActionResult Delete(int Id)
        {
            try
            {
                if (!_identityService.Authorize(StandardPermissionProvider.ManageWriteCheckOutTransaction.SystemName, CurrentUser, PermissionActions.Delete))
                    return Json(new { type = "faild", msg = _LocalizationService.GetLocalizedHtmlString("Permission.AccessDenied").Value });                

                CheckTransaction master = _CheckTransactionService.GetById(Id);

                //منع حذف حركة كتابة شيك اذا كان تم اعتمادها في حركة صرف شيك
                var CheckOutTransaction = _CheckTransactionService.GetWithCondetion(x => x.CheckNumber == master.CheckNumber && x.DocTypeId == (int)DocumentTypes.CheckOutExchangeTransaction).FirstOrDefault();
                if (CheckOutTransaction != null)
                {
                    return Json(new { type = "faild", msg = "غير مسموح بحذف الحركة لأنه تم إعتمادها في حركة صرف شيك صادر" });
        ...
        public IActionResult Print(int Id)
        {
            if (Id > 0)
            {
                WriteCheckOutRptPrint report = new WriteCheckOutRptPrint(_LocalizationService);
                var DataSource = _ReportService.WriteCheckOutRptPrint(Id);
                report.DataSource = DataSource;
                var stream = report.GenerateReport("pdf");
                stream.Position = 0;
                return File(stream, "application/pdf", "WriteCheckOutRptPrint.pdf");
            }
            else
            {
                return View("Error");
        ...
        public void AddEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, out int EntryNumber, out int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                EntryMaster.DocType = (int)DocumentTypes.WriteCheckOutTransaction;
                ...
                EntryMaster.SetBasicData(CRUD_OperationType.Create, (int)DocumentTypes.WriteCheckOutTransaction, CurrentUser);
                #endregion
        ...
        public void UpdateEntry(TransactionEntrySetting transactionSettings, int DocNumber, int CuruncyId, decimal CuruncyFactor, decimal Amount, decimal DiscountAmount, int DiscountAccountId, int EntryId)
        {
            ...
            if (transactionSettings != null)
            {
                ...
                if (oldEntryMaster != null)
                {
                    ...
                    oldEntryMaster.SetBasicData(CRUD_OperationType.Update, (int)DocumentTypes.WriteCheckOutTransaction, CurrentUser);
                    #endregion
